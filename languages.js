(function () {
  ...
  if (window.__inputTranslateLanguages) return;
  ...
  function getShippedLanguages() {
    return LANGUAGES.filter((lang) => lang.shipped);
  }

  function getShippedLanguageIds() {
    return getShippedLanguages().map((lang) => lang.id);
  }

  function isShippedLanguageId(id) {
  ...
  function mapLanguageCodes(providerId, { source, target }) {
  ...
  function getChromeProbePair(targetLanguage, sourceLanguage) {
  ...
  window.InputTranslate.languages = {
    LANGUAGES,
    getLanguage,
    getShortLabel,
    getShippedLanguages,
  ...
  window.__inputTranslateLanguages = true;
})();
