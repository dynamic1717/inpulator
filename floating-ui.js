(function () {
  ...
  function buildTranslateIcon(sourceLanguage, targetLanguage) {
    ...
    return `<svg width="32" height="32" viewBox="0 0 62 62" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
  <style>
  ...
  function create({ onTranslate }) {
    ...
    function setTranslate(
      selectedChars,
      remaining,
      { showCounter = true, sourceLanguage = 'ru', targetLanguage = 'en' } = {}
    ) {
      ensureButton();
      ...
      if (!showCounter) {
        ...
        button.title = `Перевести на ${targetName} (${pairLabel}, Alt+Shift+T)`;
        return;
      }

      const counter = `${formatCount(selectedChars)} / ${formatCount(remaining)}`;
      button.classList.add('input-translate-btn--with-counter');
      button.innerHTML = `${icon}<span class="input-translate-counter">${counter}</span>`;
      button.setAttribute('aria-label', `Перевести на ${targetName}`);
      button.title = `Перевести на ${targetName} (${pairLabel}, ${counter} символов, Alt+Shift+T)`;
    }
